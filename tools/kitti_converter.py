...
from jmodt.config import TRAIN_SEQ_ID, VALID_SEQ_ID, SMALL_VAL_SEQ_ID
...
def create_train_sample_data(input_root, output_root, init_or_clear_dirs=False, only_labels=False):
    ...
    res_dir = os.path.join(output_root, 'ImageSets')
    init_or_clear_dir(res_dir)
    ...
    with open(os.path.join(res_dir, 'train.txt'), 'w+') as f:
        for seq in TRAIN_SEQ_ID:
            sample_list = training_seq_to_sample[seq]
            for i in range(len(sample_list) - 1):
                f.write(f'{sample_list[i]} {sample_list[i + 1]}\n')

    with open(os.path.join(res_dir, 'val.txt'), 'w+') as f:
        for seq in VALID_SEQ_ID:
            sample_list = training_seq_to_sample[seq]
            for sample in sample_list:
                f.write(f'{sample}\n')

    with open(os.path.join(res_dir, 'small_val.txt'), 'w+') as f:
...
if __name__ == '__main__':
    ...
    create_train_sample_data(input_root=in_dir, output_root=out_dir, init_or_clear_dirs=True)
